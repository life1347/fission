...
func (roundTripper RetryingRoundTripper) RoundTrip(req *http.Request) (resp *http.Response, err error) {
	...
	for i := 0; i < roundTripper.funcHandler.tsRoundTripperParams.maxRetries-1; i++ {
		...
		if !ableToUpdate {
			...
			service, err := roundTripper.funcHandler.executor.GetServiceForFunction(fnMeta)
			if err != nil {
				log.Printf("Err from GetServiceForFunction : %v", err)
				// We might want a specific error code or header for fission failures as opposed to
				// user function bugs.
				log.Printf("Error getting service for function (%v), retry again: %v", fnMeta, err)
...
func (fh *functionHandler) grabUpdateEntryLock(fnMeta *metav1.ObjectMeta) (lock *updateLock, ableToUpdate bool) {
